...
class ImpalaCNN(nn.Module):
    ...
    def __init__(self, obs_space, num_outputs):
        ...
        conv_seqs = []
        for out_channels in [16, 32, 32, 32, 32]:  # 5 convolutional layers
            conv_seq = ConvSequence(shape, out_channels)
            shape = conv_seq.get_output_shape()
            conv_seqs.append(conv_seq)

        self.conv_seqs = nn.ModuleList(conv_seqs)
        self.hidden_fc1 = nn.Linear(in_features=shape[0] * shape[1] * shape[2], out_features=256)
        self.hidden_fc2 = nn.Linear(in_features=256, out_features=512)
