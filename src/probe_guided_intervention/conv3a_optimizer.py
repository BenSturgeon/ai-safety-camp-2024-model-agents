#!/usr/bin/env python
"""
Conv3a optimization for FC3 probe control.
Optimizing middle layers (conv3a) for comparison with conv2a approach.
"""
...
def compute_conv3a_fc3_loss(conv3a_modified, controller, obs_tensor, target='green_key', threshold=0.8):
    """
    Compute loss for conv3a modifications to control FC3 probes.

    Args:
        conv3a_modified: Modified conv3a activations
    ...
    # Set up hook to replace conv3a with modified version
    def replace_conv3a_hook(module, input, output):
        return conv3a_modified
    ...
    conv3a_original = getattr(controller, 'conv3a_original', conv3a_modified)
    change_penalty = 0.001 * ((conv3a_modified - conv3a_original) ** 2).mean()
    loss += change_penalty
...
def get_conv3a_activations(controller, obs_tensor):
    """Extract conv3a activations from the model."""
    activations = {}
    ...
    for name, module in controller.model.named_modules():
        if 'conv3a' in name and isinstance(module, nn.Conv2d):
            handle = module.register_forward_hook(get_activation('conv3a'))
            handles.append(handle)

    with torch.no_grad():
        _ = controller.model(obs_tensor)
        conv3a = activations.get('conv3a')
...
def compute_conv3a_multilayer_loss(conv3a_modified, controller, obs_tensor, target='green_key', fc3_weight=3.0):
    """
...
def optimize_conv3a_for_fc3(controller, obs_tensor, target_entity='green_key',
                            n_steps=50, lr=0.5, threshold=0.8):
    """
    Optimize conv3a to control FC3 predictions.
    ...
    Returns:
        optimized_conv3a, best_prob
    """

    # Get original conv3a
    conv3a_original = get_conv3a_activations(controller, obs_tensor)
    controller.conv3a_original = conv3a_original  # Store for loss computation

    # Initialize optimization
    conv3a_modified = nn.Parameter(conv3a_original.clone().requires_grad_(True))
    optimizer = torch.optim.Adam([conv3a_modified], lr=lr)

    best_prob = 0
    best_conv3a = None

    for step in range(n_steps):
        loss, metrics = compute_conv3a_fc3_loss(
            conv3a_modified, controller, obs_tensor, target=target_entity, threshold=threshold
        )
    ...
    return best_conv3a if best_conv3a is not None else conv3a_modified.detach(), best_prob


def optimize_conv3a_multilayer(controller, obs_tensor, target_entity='green_key',
                               n_steps=1000, lr=0.5, threshold=0.8, fc3_weight=3.0):
    """
    Optimize conv3a using all layer probes with FC3 weighting.
    ...
    Returns:
        optimized_conv3a, best_fc3_prob
    """

    # Get original conv3a
    conv3a_original = get_conv3a_activations(controller, obs_tensor)
    controller.conv3a_original = conv3a_original
    ...
    for step in range(n_steps):
        loss, metrics = compute_conv3a_multilayer_loss(
            conv3a_modified, controller, obs_tensor,
    ...
    return best_conv3a if best_conv3a is not None else conv3a_modified.detach(), best_fc3_prob