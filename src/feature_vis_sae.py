...
class SAEFeatureVisualizer:
    ...
    def visualize_sae_feature(self, layer_number, feature_idx, 
        ...
        self.sae_hook_handle = replace_layer_with_sae(self.model, self.sae, layer_number)
...
def visualize_sae_features(model, sae_checkpoint_path, layer_number=None, num_features=None, color_correlation_path=None, batch_size=8, num_batches=16):
    """
    ...
    for batch_idx in range(num_batches):
        ...
        try:
            ...
            fig.suptitle(f'SAE Features in {layer_name} (Batch {batch_idx+1}/{num_batches})', fontsize=16)
            ...
            output_path = os.path.join(output_dir, f'sae_filters_{layer_name}_batch{batch_idx+1}.png')
            plt.savefig(output_path, dpi=150)
            print(f"Saved batch image to: {output_path}")
            
            # Also save a copy in the current directory for backward compatibility
            plt.savefig(f'2_sae_filters_{layer_name}_batch{batch_idx+1}.png', dpi=150)
...
if __name__ == "__main__":
    ...
    visualize_sae_features(
        ...
        num_batches=1,         # Create 16 images
