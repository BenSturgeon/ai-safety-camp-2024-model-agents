...
print("running modified wandb code")
    ...
    for step_cnt in range(max_steps):
        ...
        if (step_cnt + 1) % config.nsteps == 0:
            ...
            wandb.log({'steps': step_cnt + 1,
                    ...
                    'fps': fps,
                    'num_ppo_update': num_ppo_updates,
                    'eprewmean': safe_mean([info['r'] for info in train_epinfo_buf]),
                    'eplenmean': safe_mean([info['l'] for info in train_epinfo_buf]),
                    'eval_eprewmean': safe_mean([info['r'] for info in test_epinfo_buf]),
                    'eval_eplenmean': safe_mean([info['l'] for info in test_epinfo_buf]),
